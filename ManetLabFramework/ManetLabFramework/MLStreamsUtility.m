...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)streamEvent {
    switch(streamEvent) {
        case NSStreamEventHasBytesAvailable:
            if (aStream == streamIn) {
                ...
                if(len > 0) {
                    ...
                    [[ManetLabFramework sharedInstance] logWithLevel:ASL_LEVEL_ERR andMsg:@"ML stream error: code %ld",len];
                }
            }
            break;
        ...
        case NSStreamEventErrorOccurred:
            [[ManetLabFramework sharedInstance] logWithLevel:ASL_LEVEL_ERR andMsg:@"ML stream error: %@",[[aStream streamError] localizedDescription]];
            [self error];
            break;
        default:
            break;
